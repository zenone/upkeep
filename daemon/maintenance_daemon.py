...
MAINTAIN_SH = Path("/usr/local/lib/mac-maintenance/maintain.sh")
POLL_INTERVAL = 2  # seconds
LOG_FILE = Path("/var/log/mac-maintenance-daemon.log")
...
def run_operation(operation_id: str, job_id: str = "") -> Dict[str, Any]:
    ...
    # Check maintain.sh exists
    if not MAINTAIN_SH.exists():
        result["error"] = f"maintain.sh not found at {MAINTAIN_SH}"
        log(f"maintain.sh not found: {MAINTAIN_SH}", "ERROR")
        return result
    ...
    try:
        ...
        # Set environment variables for maintain.sh
        env = os.environ.copy()
        env["MAC_MAINTENANCE_DAEMON"] = "1"  # Allow root execution from daemon
        env["MAINTAIN_SH"] = str(MAINTAIN_SH)  # Path to maintain.sh for timestamp checks

        # CRITICAL: Pass actual user context to maintain.sh
        ...
        if actual_user and actual_home:
            ...
            # Set SUDO_USER so maintain.sh's get_actual_user_home() can use it
            env["SUDO_USER"] = actual_user
            # Set HOME to root's home for logging (maintain.sh needs it for LOG_DIR)
            env["HOME"] = "/var/root"
        else:
