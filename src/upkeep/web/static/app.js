...
Error: ${l.message}
`),console.error("Error running operations:",l)}finally{o===_?(console.log("Cleaning up invocation",o,"(current)"),n&&(n.disabled=!1),i&&(i.style.display="none"),a&&(a.style.display="none"),re=!1,He(),_e(),N=0,c&&(c.style.display="none"),s&&(s.disabled=!1,s.removeAttribute("aria-disabled"),s.title=""),r&&(r.disabled=!1,r.removeAttribute("aria-disabled"),r.title="")):console.log("Skipping cleanup for invocation",o,"(superseded by",_,")")}}async function Le(){if(confirm("Skip the current operation?"))try{let t=await fetch("/api/maintenance/skip",{method:"POST"});if(!t.ok)throw new Error(`HTTP ${t.status}`);let e=await t.json(),o=document.getElementById("progress-text");if(o){let n=e.message||"Skipped by user";o.textContent+=`
