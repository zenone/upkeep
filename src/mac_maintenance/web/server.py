"""FastAPI server for mac-maintenance web interface.
...
from mac_maintenance.api import StorageAPI, MaintenanceAPI, ScheduleAPI
from mac_maintenance.core.launchd import LaunchdGenerator
from mac_maintenance.core import system as system_utils
from mac_maintenance import __version__
from mac_maintenance.web.models import (
    SystemInfoResponse,
...
Currently localhost-only. Authentication planned for remote access.
    ...
    contact={
        ...
        "url": "https://github.com/yourusername/mac-maintenance",
    },
    license_info={
        "name": "MIT",
...
Checks ~/Library/Logs/mac-maintenance for operation history.
    """,
)
async def get_last_run() -> Dict[str, Any]:
    """Get timestamp of last maintenance run from timestamp file.

    Checks the ~/Library/Logs/mac-maintenance directory for the last_run_timestamp.txt
    ...
    try:
        ...
        if not timestamp_file.exists():
            ...
            log_files = sorted(log_dir.glob("mac-maintenance-*.log"), reverse=True)
            if not log_files:
                return {
                    "success": True,
