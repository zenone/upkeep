...
def clean_output_line(text: str) -> str:
    ...
    Removes carriage returns and ANSI escape codes that can cause text
    truncation or corruption when displayed in web UI.

    Business logic:
    - Terminal output often contains ANSI codes for colors, carriage returns
    ...
    # Remove ANSI escape codes (color, cursor movement, etc.)
    # Pattern: \x1b[ followed by any characters until m
    # Example: \x1b[32m (green), \x1b[0m (reset), \x1b[2K (clear line)
...
class MaintenanceAPI(BaseAPI):
    ...
    QUEUE_DIR = Path("/var/local/mac-maintenance-jobs")
    ...
    def _load_operation_details(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Path: src/mac_maintenance/api/maintenance.py
            # Up 1: src/mac_maintenance/api
            # Up 2: src/mac_maintenance
            # Up 3: src
            # Up 4: project root
    ...
    async def run_operations(
            ...
            try:
                ...
                if success:
                    try:
                        ...
                        self._logger.warning(f"Failed to write operation history: {e}")
        ...
        try:
            ...
            self._logger.warning(f"Failed to write timestamp file: {e}")
    ...
    def skip_current_operation(self) -> bool:
        ...
        try:
            ...
            self._logger.info("Skip flag written, daemon will kill current subprocess")
            return True
        except Exception as e:
            self._logger.error(f"Failed to write skip flag: {e}")
            return False
        ...
        try:
            ...
            if status_file.exists():
                try:
                    ...
                    self._logger.warning(f"Failed to read status file: {e}")
